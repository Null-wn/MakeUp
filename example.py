...
var_usr_name.set('username')  # 变量赋值
...
def usr_modify():
    def sign_to_app():
        ...
        ##如果用户名没有在我们的数据文件中，则提示`'Error', 'The user has already signed up!'`
        elif nn not in exist_usr_info:
            tk.messagebox.showerror('Error', '这个用户名没有注册！')
        ...
        ##然后销毁窗口。
            ...
            with open('usrs_info.pickle', 'wb') as usr_file:
                pickle.dump(exist_usr_info, usr_file)
            tk.messagebox.showinfo('Welcome', '你成功修改密码了!')
            ##然后销毁窗口。
            window_sign_up.destroy()

    window_sign_up = tk.Toplevel(window)
    window_sign_up.geometry('350x200')
    window_sign_up.title('修改密码')
    # 新用户名
    new_name = tk.StringVar()  # 将输入的注册名赋值给变量
    #new_name.set('example@python.com')  # 将最初显示定为'example@python.com'
    tk.Label(window_sign_up, text='用户名: ').place(x=10, y=10)  # 将`User name:`放置在坐标(10,10)。
    ...
    entry_usr_pwd_confirm.place(x=150, y=90)
    # 下面的 sign_to_app
    btn_comfirm_sign_up = tk.Button(window_sign_up, text='修改', command=sign_to_app)
    btn_comfirm_sign_up.place(x=150, y=130)
...
btn_modify = tk.Button(window, text='修改密码', command=usr_modify)
btn_modify.place(x=250, y=230)
##显示出来
window.mainloop()
