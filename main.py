...
def bg1_callback(select):
    select=1

def bg2_callback(select):
    select=2
    print(select)

def bg3_callback(select):
    select=3
...
#一级菜单
editmenu = tk.Menu(menubar, tearoff=0)
menubar.add_cascade(label='更换背景', menu=editmenu)
# 二级菜单
editmenu.add_command(label='图片1', command=bg1_callback(select))
editmenu.add_command(label='图片2', command=bg2_callback(select))
editmenu.add_command(label='图片3', command=bg3_callback(select))

root.config(menu=menubar)
print(select)
if select==1:
    backgroundImg=resize_pic2(current_path + "\\imgs\\background1.jpg")
if select==2:
